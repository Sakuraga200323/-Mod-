...
public class ShooterBlackTrigger extends Item {

    String name = "shooter_blacktrigger";
    ...
    float[][] bulletsStatus2 = {
            ...
            {1.00f, 6.00f, 0.00f, 0.01f, 1.00f},
            {1.50f, 8.00f, 0.00f, 0.01f, 3.00f},
            {1.00f, 4.00f, 1.00f, 0.01f, 1.00f},
            {1.50f, 4.50f, 2.00f, 0.01f, 2.00f},
            {0.75f, 4.00f, 0.00f, 1.50f, 3.00f},
            {1.00f, 4.00f, 1.00f, 1.50f, 4.00f},
    };
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn){
        ...
        if (playerIn.isSneaking() == true) {
            // メインハンド処理
            if (item instanceof ShooterBlackTrigger) {
                ...
                if (!worldIn.isRemote) {
                    ...
                    if (itemStack.getMaxDamage() > (itemStack.getItemDamage() + bulletCost)) {
                        ...
                        x = playerIn.posX - 0.4 * Math.sin(TMath.addRotationYawNumRad(playerIn, (float) temp))*Math.sin(-TMath.getRotationPitchRad(playerIn));
                        y = playerIn.posY + 0.8d + Math.sin(-TMath.getRotationPitchRad(playerIn));
                        z = playerIn.posZ + 0.4 * Math.cos(TMath.addRotationYawNumRad(playerIn, (float) temp))*Math.sin(-TMath.getRotationPitchRad(playerIn));

                        bullet = new EntityGuidedBullet(worldIn, playerIn, x, y, z, playerIn.getRotationYawHead(), playerIn.rotationPitch);
                        bullet.particleGuiding = EnumParticleTypes.DRAGON_BREATH;
                        bullet.particleNotGuiding = EnumParticleTypes.DRAGON_BREATH;
                        bullet.particleDead = EnumParticleTypes.END_ROD;
    ...
    public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
        ...
        if (isSelected || stack == entity.getHeldItemOffhand()) {
            ...
            if (this.tickToHealItem >= 40 && stack.getItemDamage() >= 1) {
                if (!worldIn.isRemote) {
                    stack.setItemDamage(getDamage(stack) - 2);
