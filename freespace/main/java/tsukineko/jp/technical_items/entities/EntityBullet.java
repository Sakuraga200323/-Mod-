...
public class EntityBullet extends Entity implements IProjectile {
    ...
    public EntityBullet(World worldIn, EntityPlayer shooter) {
        super(worldIn);
        this.setSize(0.5f, 0.5f);
        this.shooter = shooter;
    }

    public EntityBullet(World worldIn, EntityPlayer shooter, double x, double y, double z) {
        super(worldIn);
        this.setSize(0.5f, 0.5f);
        this.shooter = shooter;
    ...
    public EntityBullet(World worldIn, EntityPlayer shooter, double x, double y, double z, float rotationYaw, float rotationPitch) {
        super(worldIn);
        this.setSize(0.5f, 0.5f);
        this.shooter = shooter;
    ...
    public EntityBullet(World worldIn, EntityPlayer shooter, double x, double y, double z, float rotationYaw, float rotationPitch, float speed, float damage) {
        super(worldIn);
        this.setSize(0.5f, 0.5f);
        this.shooter = shooter;
        this.setPositionAndRotation(x, y, z, rotationYaw, rotationPitch);
    ...
    protected void hitSomethingOrNaturalDead(EntityBullet selfEntity){
        // ≪particleDead≫パーティクル
        ((WorldServer) this.world).spawnParticle(this.particleDead, true, this.posX, this.posY, this.posZ, 1, 0.0D, 0.0D, 0.0D, 0.0D,2 );
        // 爆発サイズがあれば爆発するやつ
    ...
    protected Entity findEntityOnPath(Vec3d start, Vec3d end){
        Entity entity = null;
        List<Entity> list = this.world.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox().expand(this.motionX, this.motionY, this.motionZ).grow(1.0D), AMMO_TARGETS);
        double d0 = 0.0D;
